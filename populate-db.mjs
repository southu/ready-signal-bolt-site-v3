...
const SUPABASE_URL = 'https://sniafkavdcrlshjmqufa.supabase.co';
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNuaWFma2F2ZGNybHNoam1xdWZhIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njc5ODAzOTUsImV4cCI6MjA4MzU1NjM5NX0.2ZJvAmsJPdJbLkvW_BetY4xir7OP3fqnbxWfjyip2ro';
...
const helpArticles = [
  ...
  },
  {
    ...
    content: 'Yeo-Johnson Power Transformation is a statistical method to normalize data distributions. Works with both positive and negative values, making it more versatile than Box-Cox transformation. The Yeo-Johnson transformation is particularly useful when your data contains zero or negative values.',
    url: '/what-is-a-yeo-johnson-power-transformation/',
    category: 'Data Science Treatments',
    tags: ['transformation', 'yeo-johnson', 'yeo', 'johnson', 'normalization', 'statistics']
  },
  {
  ...
  },
  {
    ...
    category: 'Data Science Treatments',
    tags: ['transformation', 'box-cox', 'box', 'cox', 'variance', 'statistics']
  },
  {
...
async function populateDatabase() {
  console.log('🔄 Populating database with', helpArticles.length, 'articles...');

  const { data, error } = await supabase
    .from('help_articles')
    .upsert(helpArticles, { onConflict: 'url' });
  ...
  console.log('✅ Success! Database populated with', helpArticles.length, 'articles.');

  // Test a search for "yeo"
  console.log('\n🔍 Testing search for "yeo"...');
