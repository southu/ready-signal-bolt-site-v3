...
// Single source of truth: src/pages/landing/forecastingTestimonials.json — the
// same JSON the React carousel imports via forecastingLandingContent.ts. Loading
// it here keeps the prerendered static body (BUG-1: the raw HTML crawlers/curl
// see) in lockstep with the live carousel, so there is no hand-maintained copy
// to drift or re-truncate. React replaces this markup with the live carousel
// once it mounts in the browser.
const FORECASTING_TESTIMONIALS = JSON.parse(
  readFileSync(
    join(__dirname, '..', 'src', 'pages', 'landing', 'forecastingTestimonials.json'),
    'utf-8',
  ),
);
